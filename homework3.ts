...
*/
/* 
function countFunc(para :string | number) :number {
    const getPara = para.toString();
    return getPara.length;
}
console.log(countFunc(1212112)); */
...
type MerryType = {income : number, home : boolean, charm : '상' | '중' | '하'};
function merryFunc({income,home,charm}:MerryType) :string {
    let sum :number = 0;
